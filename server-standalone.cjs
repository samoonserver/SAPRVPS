...
class RTMPStreamManager {
  ...
  startUptimeTracking() {
    ...
    this.uptimeInterval = setInterval(async () => {
      if (this.streamStartTime) {
        ...
        try {
          await db.query('UPDATE stream_status SET uptime = $1 WHERE id = (SELECT MAX(id) FROM stream_status)', [uptime]);
        } catch (error) {
  ...
  async updateStreamStatus(status, videoId) {
    try {
      await db.query(
        'UPDATE stream_status SET status = $1, current_video_id = $2, started_at = $3 WHERE id = (SELECT MAX(id) FROM stream_status)',
